...
int32_t computeRequiredCoreCount(const GraphImage *graph) {
  if (!graph)
    return 0;

  int32_t coreCount = 0;
  for (uint32_t i = 0; i < graph->task_count; ++i) {
    coreCount = std::max(coreCount, graph->tasks[i].core_id);
  }
  return coreCount;
}
...
void destroyRuntimeHandle(RuntimeHandle *runtime) {
  ...
  if (runtime->graph && computeRequiredCoreCount(runtime->graph) > 0)
    analog::golem_sim::shutdownBridge();
...
int32_t runTaskRange(RuntimeHandle *runtime, uint32_t begin, uint32_t end) {
  ...
  for (uint32_t i = begin; i < end; ++i) {
    ...
    TaskCall *taskCall = static_cast<TaskCall *>(task.opaque);
    int32_t coreId =
        (taskCall && taskCall->task) ? taskCall->task->core_id : kTaskCoreIdNone;
    if (coreId >= 1)
      analog::golem_sim::setActiveCore(coreId - 1);

    int32_t rc = task.fn(task.opaque);
    if (coreId >= 1)
      analog::golem_sim::clearActiveCore();
    if (rc != 0)
      return rc;
  }
...
RuntimeHandle *analog_runtime_init(GraphImage *graph) {
  ...
  int32_t requiredCoreCount = computeRequiredCoreCount(graph);
  if (requiredCoreCount > 0)
    analog::golem_sim::initializeBridge(requiredCoreCount, -1, -1, -1);
