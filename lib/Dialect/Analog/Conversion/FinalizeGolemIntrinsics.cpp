...
std::optional<ShimKind> classifyShimName(StringRef name) {
  return llvm::StringSwitch<std::optional<ShimKind>>(name)
      .Case(golem::kSetIntrinsicName, ShimKind::Set)
      .Case(golem::kLoadIntrinsicName, ShimKind::Load)
      .Case(golem::kComputeIntrinsicName, ShimKind::Compute)
      .Case(golem::kStoreIntrinsicName, ShimKind::Store)
      .Default(std::nullopt);
}
...
LogicalResult verifyShimCall(LLVM::CallOp call, StringRef calleeName,
  ...
  if (call.getNumResults() != 0)
    return call.emitError() << "expected void shim call for '" << calleeName
                            << "'";
  ...
  case ShimKind::Compute:
    if (operandTypes.size() != 1) {
      return call.emitError()
             << "expected one operand for '" << calleeName << "', got "
             << operandTypes.size();
    }
  ...
  case ShimKind::Load:
    if (operandTypes.size() != 8) {
      return call.emitError()
             << "expected 8 operands for '" << calleeName << "', got "
             << operandTypes.size();
    }
    ...
    for (unsigned index = 2; index < 7; ++index) {
      if (failed(
              verifyOperandType(call, operandTypes[index], isI64, "i64", index)))
        return failure();
    }
  ...
  case ShimKind::Store:
    if (operandTypes.size() != 10) {
      return call.emitError()
             << "expected 10 operands for '" << calleeName << "', got "
             << operandTypes.size();
    }
    if (failed(verifyOperandType(call, operandTypes[0], isLLVMOpaquePointer,
                                 "!llvm.ptr", 0)) ||
...
Value buildEffectiveDataPointer(IRRewriter &rewriter, Location loc, Value dataPtr,
                                Value offset) {
...
void eraseDeadShimDecls(ModuleOp module) {
  ...
  for (StringRef name : {StringRef(golem::kSetIntrinsicName),
                         StringRef(golem::kLoadIntrinsicName),
                         StringRef(golem::kComputeIntrinsicName),
                         StringRef(golem::kStoreIntrinsicName)}) {
