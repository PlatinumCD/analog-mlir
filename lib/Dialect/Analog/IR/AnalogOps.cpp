  ...
  for (Value resource : resources) {
    if (!isValidTaskResource(taskOp, resource)) {
      return taskOp.emitOpError("expected ")
             << name << " to be produced by analog.task_graph.input, "
             << "analog.task_graph.output, or "
             << "analog.task_graph.temporary, or "
             << "analog.task_graph.persistent in the same graph";
    }
  }
...
mlir::LogicalResult mlir::analog::TaskCreateOp::verify() {
  // Validate the task's execution placement before checking graph topology.
  if (!isValidTaskDomain(getDomain())) {
    return emitOpError("expected domain to be either \"analog\" or \"digital\"");
  }
  ...
  for (Value dependency : getDependencies()) {
    ...
    if (!dependencyTask) {
      return emitOpError(
          "expected dependencies to be produced by analog.task.create");
    }
