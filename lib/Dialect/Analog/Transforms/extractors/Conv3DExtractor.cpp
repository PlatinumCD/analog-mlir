...
// Carries the ops and boundary values that make up one Conv3D layer slice.
struct Conv3DMatch {
...
// Collects the matched Conv3D slice in clone order for outlining.
static void collectMatchedOps(Conv3DMatch &match) {
  match.ops.clear();
...
static void finalizeConv3DMatch(Conv3DMatch &match) {
...
static std::optional<Conv3DMatch> matchConv3DWithBias(mlir::Operation *op) {
  // Anchor the match on the linalg Conv3D op and its expected operand shape.
  if (!op || op->getName().getStringRef() != "linalg.conv_3d_ncdhw_fcdhw")
    return std::nullopt;
  ...
  // Require exactly one static weight among the two convolution inputs.
...
// Outlines the matched Conv3D slice and tags the new layer by bias form.
static void rewriteConv3DExtractor(const Conv3DMatch &match,
                                   mlir::RewriterBase &rewriter) {
  mlir::StringRef layerType =
      match.outputBroadcast ? mlir::StringRef("conv3d_w_bias")
                            : mlir::StringRef("conv3d");
  rewrite_utils::extractToFunction(match.root, match.ops, match.inputs,
                                   match.outputs, rewriter, layerType);
}

// Finds three-dimensional convolution layers and outlines each match.
class Conv3DExtractor : public mlir::analog::LayerExtractor {
public:
  ...
  void extract(mlir::func::FuncOp func) const override {
    ...
    extractor_impl::extractAllMatches(func, rewriter, matchConv3DWithBias,
                                      rewriteConv3DExtractor);
    extractor_impl::extractAllMatches(func, rewriter, matchConv3DWithoutBias,
                                      rewriteConv3DExtractor);
  }
};
