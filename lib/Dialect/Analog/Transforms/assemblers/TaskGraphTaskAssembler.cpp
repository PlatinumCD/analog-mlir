...
std::string resolveTaskDomain(mlir::ModuleOp module, mlir::func::CallOp call) {
  ...
  auto layerDomain = calleeFunc->getAttrOfType<mlir::StringAttr>("layer_domain");
  if (!layerDomain)
    return "digital";

  if (layerDomain.getValue() == "analog" || layerDomain.getValue() == "digital")
    return layerDomain.getValue().str();

  return "digital";
}

mlir::StringAttr resolveTaskLayerType(mlir::ModuleOp module,
                                      mlir::func::CallOp call) {
  auto callLayerType = call->getAttrOfType<mlir::StringAttr>("layer_type");
  if (callLayerType && !callLayerType.getValue().empty())
    return callLayerType;
  ...
  auto layerType = calleeFunc->getAttrOfType<mlir::StringAttr>("layer_type");
  if (!layerType || layerType.getValue().empty())
    return {};
...
collectTaskDependencies(mlir::func::CallOp call, mlir::ModuleOp module,
    ...
    if (calleeFunc) {
      ...
      if (weightDependencies) {
        for (mlir::Attribute attr : weightDependencies) {
          ...
          auto dependencyCallIt = callByCallee.find(dependencyName.getValue());
          ...
          sourceCalls.insert(dependencyCallIt->second);
        }
      }
    }
  }
...
collectPersistentResourceBySymbol(mlir::func::FuncOp taskGraphFunc) {
  ...
  for (mlir::Operation &op : taskGraphFunc.getBody().front()) {
    ...
    auto persistentSymbol = op.getAttrOfType<mlir::StringAttr>(
        mlir::analog::assembler_utils::kPersistentSymbolAttrName);
    ...
    auto insertIt =
        resourceBySymbol.try_emplace(persistentSymbol.getValue(),
                                     persistentResource.getResult());
    if (!insertIt.second) {
...
void collectOrderedCalls(mlir::Block &forwardBlock,
...
mlir::LogicalResult collectResourceByValue(
  ...
  for (mlir::Operation &op : taskGraphFunc.getBody().front()) {
    if (auto inputResource = llvm::dyn_cast<mlir::analog::TaskGraphInputOp>(&op)) {
      auto inputIndex = getIndexAttrValue(
          &op, mlir::analog::assembler_utils::kForwardInputIndexAttrName);
    ...
    if (auto outputResource =
            llvm::dyn_cast<mlir::analog::TaskGraphOutputOp>(&op)) {
      auto outputIndex = getIndexAttrValue(
          &op, mlir::analog::assembler_utils::kForwardOutputIndexAttrName);
    ...
    if (auto temporaryResource =
      ...
      auto callIndex = getIndexAttrValue(
          &op, mlir::analog::assembler_utils::kForwardCallIndexAttrName);
      if (failed(callIndex))
        return mlir::failure();

      auto resultIndex = getIndexAttrValue(
          &op, mlir::analog::assembler_utils::kForwardResultIndexAttrName);
      ...
      resourceByValue[call.getResult(*resultIndex)] = temporaryResource.getResult();
    }
  }
...
public:
  ...
  mlir::LogicalResult assemble(mlir::ModuleOp module,
    ...
    auto graph = mlir::analog::assembler_utils::findTaskGraphCreateOp(taskGraphFunc);
    if (!graph) {
      taskGraphFunc.emitError("expected generated task graph function to begin "
                              "with analog.task_graph.create");
      return mlir::failure();
    }
    ...
    mlir::Block &forwardBlock = forward.getBody().front();
    auto returnOp = llvm::dyn_cast<mlir::func::ReturnOp>(forwardBlock.getTerminator());
    if (!returnOp) {
    ...
    for (mlir::func::CallOp call : orderedCalls) {
      ...
      if (call.getNumResults() > 1) {
        call.emitError("task graph runtime ABI supports at most one task "
                       "output; split or rewrite this helper before task "
                       "assembly");
        return mlir::failure();
      }

      std::string domain = resolveTaskDomain(module, call);
      mlir::StringAttr layerType = resolveTaskLayerType(module, call);
      if (auto calleeFunc =
              module.lookupSymbol<mlir::func::FuncOp>(calleeAttr.getValue())) {
      ...
      if (calleeFunc) {
        ...
        if (weightDependencies) {
          for (mlir::Attribute attr : weightDependencies) {
            ...
            auto persistentIt =
                persistentResourceBySymbol->find(dependencyName.getValue());
      ...
      for (mlir::Value result : call.getResults()) {
        auto resourceIt = resourceByValue.find(result);
        if (resourceIt == resourceByValue.end())
          continue;
      ...
      if (layerType && layerType.getValue() == "weight_init") {
        auto persistentIt =
            persistentResourceBySymbol->find(calleeAttr.getValue());
        if (persistentIt == persistentResourceBySymbol->end()) {
          call.emitError("could not find persistent resource for weight init "
                         "callee '")
              << calleeAttr.getValue() << "'";
          return mlir::failure();
        }

        outputResources.push_back(persistentIt->second);
      }

      auto task = rewriter.create<mlir::analog::TaskCreateOp>(
          call.getLoc(), graph.getResult(), calleeAttr,
          rewriter.getStringAttr(domain), layerType, inputResources,
          outputResources, *dependencies);
