...
public:
  ...
  mlir::LogicalResult assemble(mlir::ModuleOp module,
    ...
    auto taskGraphType = mlir::analog::TaskGraphType::get(rewriter.getContext());
    auto functionType =
    ...
    rewriter.create<mlir::analog::TaskGraphCreateOp>(forward.getLoc(),
                                                     taskGraphType);

    mlir::analog::assembler_utils::setGeneratedTaskGraphFunc(forward,
                                                             generateFunc);
    return mlir::success();
  }
};
